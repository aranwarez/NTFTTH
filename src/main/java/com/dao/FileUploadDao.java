...
public class FileUploadDao {
	...
	public static String saveUpload(String DESCRIPTION,String FILE_LOCATION,String ROLE_CODE, String DISPLAY_FLAG,String USER) throws SQLException {
        Connection con = DbCon.getConnection();
        try {
	...
	public String maxOneActiveFile(String ROLE_CODE) throws SQLException {
    	Connection con = DbCon.getConnection();
		  try {
			  PreparedStatement pst=con.prepareStatement("select file_location from FILE_NOTIFICATION where upload_id=(select max(upload_id)upload_id from FILE_NOTIFICATION where display_flag='Y' and ROLE_CODE= NVL(?, ROLE_CODE)) and ROLE_CODE= NVL(?, ROLE_CODE)");
